...
- [x] Compose renders with a completed environment file and selects the intended images.
- [x] Preparation and initialization jobs finish with exit code 0.
- [x] Every long-running container reports healthy, with no unexpected exits or restarts.
- [x] The application responds on its published endpoint and passes the recorded smoke check.
- [x] Startup and steady-state logs have been reviewed; no unresolved application errors remain.
- [x] Restarting the stack preserves its initialized data and returns it to healthy status.
- [x] External integrations tested with real credentials are identified separately from placeholder checks.
- [x] Temporary containers and test data are removed after evidence is recorded.
...
All 44 final variants passed local startup, application and retained-data lifecycle checks. Final log audits found no unresolved error-level events or unhandled tracebacks. This does not mean silent logs: documented warnings and Baserow's INFO socket-close diagnostic remain.

`Pending` means unverified. `Blocked` means the success criteria were not met. A caveat does not turn a failed check into a pass.
...
| baserow | Local checks passed; INFO diagnostic | Local checks passed; INFO diagnostic | [Batch B](batch-b.md). Full template import, authenticated table/row operations and persistence passed. Gunicorn 24.1 removed ERROR-level shutdown messages; an INFO socket-close diagnostic remains documented. |
...
| navidrome | Local checks passed | Local checks passed | [Functional checks](functional-checks.md). Administrator setup, generated WAV scan, exact streamed bytes and retained-data restart passed. |
...
| qbittorrent | Local checks passed | Local checks passed | [Functional checks](functional-checks.md). Authenticated API, category/settings persistence and application logs passed; no public torrents used. |
...
| syncthing | Local checks passed | Local checks passed | [Functional checks](functional-checks.md). Standard/DHI local peers transferred generated files both ways across a retained-data restart; hashes matched. |
...
Recorded on 2026-09-09 on Linux amd64, Docker Engine 29.6.0 and Compose 5.1.2, using the rootless daemon. Large test deployments use disk-backed directories; this host mounts `/tmp` in RAM.

These checks cover isolated local deployments on this host. Placeholder credentials can verify startup where supported, but cannot verify authenticated third-party operations, email delivery, public TLS, or production traffic. DHI folders may retain upstream images for services with no configured hardened equivalent.
