...
This repository installs Proxmox VE on a Hetzner dedicated server through Linux Rescue and QEMU with legacy BIOS. Read `README.md` and `skills/install-proxmox-hetzner/SKILL.md` before operating on a server.
...
- Keep private checkpoints in the git-ignored `records/` directory: inventory, source ISO/checksum, validation, installation result, installed-guest checks, physical boot, update/reboot checks, and unresolved issues. Copy evidence off Rescue before reboot.
