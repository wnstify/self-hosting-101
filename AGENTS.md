...
- Existing approval remains valid unless the target or destructive scope changes. A prior deployment record does not authorize another deployment.
...
- Store private checkpoints in the git-ignored `records/` directory at the repository root, even when working from a topic folder.
