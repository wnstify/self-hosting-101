...
The root `skills/` directory holds agent checklists. Each skill must locate its companion topic directory and use the same scripts as the guide. `.claude/skills/` and `.agents/skills/` hold pointer files for Claude Code and Codex discovery. Keep each pointer's name and description in step with the shared skill and put no procedure in it. The root `records/` directory is git-ignored and holds private deployment evidence.
