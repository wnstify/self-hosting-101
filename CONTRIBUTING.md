...
Inspect exported pages and slides before adding them. Check readability, working links, and screenshots for private information. Do not mark a material as available before its file or public link exists.
...
Run on the workstation from the repository root:

```text
python infrastructure/proxmox-hetzner/tests/test-firmware-policy.py
```

Use Bash syntax checks for its shell scripts. The local Python tests check launcher refusals without disks or server access. The live preflight test has separate prerequisites in the [installation guide](infrastructure/proxmox-hetzner/README.md) and is not a local documentation check.
